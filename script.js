...
function migrateState(old){
 ...
 const chats=old.chats.map(c=>({id:String(c.id??Date.now()+Math.random()),name:c.name||"Новый чат",messages:Array.isArray(c.messages)?c.messages.map(m=>Array.isArray(m)?m:[m.role||"assistant",m.text||""]).filter(m=>m[1]):[],riddle:null}));
 return {...defaultState,chats:chats.length?chats:defaultState.chats,activeChatId:String(old.activeChatId??chats[0]?.id??defaultState.chats[0].id)};
}
function save(){localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}
function activeChat(){return state.chats.find(c=>c.id===state.activeChatId)||state.chats[0]}
function render(){
 const chat=activeChat();chatTitle.textContent=chat.name;chatList.innerHTML="";
 state.chats.forEach(item=>{const b=document.createElement("button");b.className="chat-item"+(item.id===state.activeChatId?" active":"");b.textContent=item.name;b.onclick=()=>{state.activeChatId=item.id;save();render()};chatList.appendChild(b)});
 messages.innerHTML="";chat.messages.forEach(([role,text])=>{const d=document.createElement("div");d.className="msg "+role;d.textContent=text;messages.appendChild(d)});
 messages.scrollTop=messages.scrollHeight;applyBackground();$("voiceEnabled").checked=!!state.voiceEnabled;
}
function applyBackground(){
 if(state.backgroundType==="image"){document.body.style.backgroundImage="url(\""+state.backgroundValue+"\")";document.body.style.backgroundColor="#f4f0ff"}
 else document.body.style.backgroundImage=backgroundPresets[state.backgroundValue]||backgroundPresets[DEFAULT_BG];
}
function addMessage(role,text,speak=true){activeChat().messages.push([role,text]);save();render();if(role==="assistant"&&speak)speakText(text)}
...
function giveUp(){const r=currentRiddle();if(!r){addMessage("assistant","Сейчас нет активной загадки 😊");return}r.stage="solved";save();addMessage("assistant","🏳️ Ответ: "+r.a[0]+". Ничего страшного! Хочешь ещё одну загадку?")}
...
function ordinaryReply(text){
 const x=normalize(text);
 if(currentRiddle()&&!["подсказка","дай подсказку","намек","подскажи","я сдаюсь","сдаюсь"].some(k=>x===k||x.includes(k))){if(checkRiddle(text))return}
 if(x.includes("я сдаюсь")||x==="сдаюсь")return giveUp();
 if(x==="подсказка"||x.includes("дай подсказку")||x.includes("намек")||x.includes("подскажи"))return giveHint();
 if(x.includes("загад"))return setRiddle();
 if(x.includes("истори"))return story();
 if(x.includes("поговор"))return addMessage("assistant","Конечно 💜 О чём хочешь поговорить?");
 if(x.includes("ассаляму алейкум")||x.includes("салам алейкум")||x.includes("салям алейкум"))return addMessage("assistant","Уа алейкум ассалям уа рахматуллахи уа баракатух! 🌷");
 if(x.includes("джазакилляху хейрон")||x.includes("джазакиллаху хейран"))return addMessage("assistant","Ваияки! 🌸");
 if(x==="спасибо"||x.includes("благодар"))return addMessage("assistant","Джазакилляху хейрон! 🌷");
 if(x==="пока"||x.includes("до свидания")||x.includes("увидимся"))return addMessage("assistant","Пока! 🌸 Пусть у тебя будет хороший день. Ассаляму алейкум!");
 if(x.includes("кто тебя создал")||x.includes("кто тебя сделал"))return addMessage("assistant","Меня создала Деккушева Джамиля 💜");
 if(x.includes("кто ты"))return addMessage("assistant","Я — "+state.assistantName+" 🌷");
 if(x.includes("дурак")||x.includes("туп")||x.includes("идиот"))return addMessage("assistant","Давай без обидных слов 🌷 Я всё равно постараюсь спокойно помочь.");
 if(x.includes("привет"))return addMessage("assistant","Ассаляму алейкум уа рахматуллахи уа баракатух! 🌷");
 if(x.includes("как дела"))return addMessage("assistant","Альхамдулиллях, хорошо 😊 А как у тебя дела?");
 if(x.includes("что ты умеешь")||x.includes("что умеешь"))return addMessage("assistant","Я умею разговаривать, придумывать истории и загадки, давать подсказки, запоминать твои чаты и помогать простыми ответами. 💜");
 return addMessage("assistant","Я тебя слушаю 💜 Рассказывай или спрашивай что-нибудь.");
}
composer.onsubmit=e=>{e.preventDefault();const text=input.value.trim();if(!text)return;addMessage("user",text,false);input.value="";setTimeout(()=>ordinaryReply(text),180)};
