...
function illnessReply(text){
  ...
  if(phrases.some(p=>x===p||x.startsWith(p+" ")||x.includes(" "+p+" "))){
    addMessage("assistant","Ох, как жаль. Да исцелит тебя Аллах. А что у тебя конкретно болит? Я хочу помочь."); return true;
  }
  return false;
}

function addHealthResponse(text){addMessage("assistant",text);return true;}
function healthReply(text){
  const x=normalize(text);

  if(x.includes("горл")){
    addHealthResponse("Если болит горло, обычно помогают отдых, питьё воды и тёплые или прохладные напитки, мягкая еда и отсутствие дыма. Если тяжело дышать или трудно глотать, состояние быстро ухудшается или есть признаки обезвоживания, нужно срочно обратиться за медицинской помощью.");
  }
  if(x.includes("живот")||x.includes("животик")||x.includes("животе")){
    addHealthResponse("При боли в животе лучше отдохнуть и понемногу пить воду. Еду выбирай лёгкую, если хочется есть. Если боль сильная или появилась внезапно, живот очень болит при прикосновении, есть кровь в рвоте или стуле, сильная рвота или становится резко хуже, нужна срочная медицинская помощь.");
  }
  if(x.includes("голов")||x.includes("голова")){
    addHealthResponse("При обычной головной боли могут помочь вода, отдых, спокойная обстановка и регулярная еда. Стоит сделать перерыв от экрана. Если боль внезапно очень сильная, появилась после травмы головы, есть слабость, онемение, спутанность, проблемы с речью или зрением, судороги или сильная рвота, нужна срочная медицинская помощь.");
  }
  if(x.includes("ног")||x.includes("нога")||x.includes("колен")||x.includes("лодыж")||x.includes("стоп")){
    addHealthResponse("Если нога заболела после нагрузки или небольшой травмы, лучше дать ей отдых и не нагружать через сильную боль. Если есть сильный или нарастающий отёк, трудно наступать на ногу, она сильно деформирована, онемела, стала синей или очень холодной, нужно обратиться за срочной медицинской помощью.");
  }
  if(x.includes("температур")||x.includes("жар")||x.includes("лихорад")){
    addHealthResponse("При температуре важно отдыхать и пить достаточно жидкости. Следи за самочувствием. Если температура очень высокая, человек становится сонным или спутанным, трудно дышать, есть судороги, сильная сыпь или состояние быстро ухудшается, нужна срочная медицинская помощь.");
  }
  if(x.includes("кашл")){
    addHealthResponse("При кашле обычно помогают отдых и достаточное питьё. Избегай дыма и сильных раздражителей. Если трудно дышать, есть боль в груди, кровь при кашле или состояние быстро ухудшается, нужна медицинская помощь.");
  }
  if(x.includes("насмор")||x.includes("заложен нос")||x.includes("нос залож")){
    addHealthResponse("При насморке помогают отдых, питьё и промывание носа физиологическим раствором. Если становится трудно дышать, появляется сильная боль или состояние заметно ухудшается, лучше обратиться к врачу.");
  }
  if(x.includes("ух")||x.includes("ухо")||x.includes("уши")){
    addHealthResponse("При боли в ухе лучше сообщить взрослому и обратиться к врачу, особенно если боль сильная, есть выделения из уха, высокая температура или ухудшился слух. Не стоит самостоятельно засовывать что-либо в ухо.");
  }
  if(x.includes("зуб")||x.includes("зубы")){
    addHealthResponse("При зубной боли лучше как можно скорее обратиться к стоматологу. До осмотра можно аккуратно прополоскать рот водой и не жевать больной стороной. Если быстро растёт отёк лица или становится трудно дышать или глотать, нужна срочная помощь.");
  }
  if(x.includes("тошнит")||x.includes("тошнот")){
    addHealthResponse("При тошноте лучше отдыхать и пить жидкость маленькими глотками. Если не получается удерживать воду, есть кровь в рвоте, сильная боль в животе или состояние ухудшается, нужна медицинская помощь.");
  }
  if(x.includes("рвот")){
    addHealthResponse("При рвоте важно понемногу пить, чтобы не допустить обезвоживания. Если рвота повторяется и вода не удерживается, есть кровь, зелёная рвота, сильная боль в животе или сильная вялость, нужна срочная медицинская помощь.");
  }
  if(x.includes("диаре")||x.includes("понос")){
    addHealthResponse("При диарее главное — пить достаточно жидкости маленькими порциями и отдыхать. Если есть кровь в стуле, сильная боль, выраженное обезвоживание или диарея долго не проходит, нужна медицинская помощь.");
  }
  if(x.includes("сып")||x.includes("пятн")||x.includes("высып")){
    addHealthResponse("При новой сыпи лучше сообщить взрослому и наблюдать за самочувствием. Если сыпь не бледнеет при надавливании и одновременно есть высокая температура, сильная вялость, проблемы с дыханием или быстрое ухудшение, нужна срочная медицинская помощь.");
  }
